...
def L(col, y):  # as in (2.5) on p29 of Fundamentals of Predictive Text Mining
    ...
    for i in range(N):
        _JF[col[i], y[i]] += 1
    JF = np.array([_JF[0, 0], _JF[0, 1], _JF[1, 0], _JF[1, 1]]).reshape(2, 2)  # joint frequencies
...
class InformationGainTransformer:
    ...
    def fit(self, X, y, names):
        ...
        if T.shape[1] != len(names):
            raise ValueError("Incompatible shapes: X %s names %d" %
                             (repr(X.shape), len(names)))
        if T.shape[0] != len(y):
            raise ValueError("Incompatible shapes: X %s y %r" % (repr(X.shape), y.shape))
        self._information_gain = np.array([L(T[:, k].toarray().squeeze(), y)
    ...
    def sorted_dataframe(self):
        if self._information_gain is None:
            raise ValueError("InformationGainTransformer().fit needs to be "
                             "called before sorted_dataframe()")
        return pd.DataFrame(data=sorted(list(zip(self._information_gain, self.names)),
    ...
    def indices_till_rank(self, rank):
        ...
        if rank > 0:
            return np.array([idx for idx,
                             r in list(enumerate(self._ranked)) if r <= rank])
        else:
            max_rank = max(self._ranked)
            return np.array([idx for idx,
                             r in list(enumerate(self._ranked)) if r >= (max_rank + rank + 1)])
